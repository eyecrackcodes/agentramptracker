...
export async function POST(request: Request) {
  try {
    ...
    const { agent_id, call_date, call_type, script_adherence, notes } = body;
    ...
    console.log(
      "About to save to database with closing_score =",
      numericScriptAdherence
    );
    ...
    // Add more logging
    console.log(
      "Will attempt insertion with closing_score =",
      numericScriptAdherence
    );
    ...
    try {
      ...
      console.log(
        "Trying to create call score using Prisma adapter with data:",
        createData
      );
    ...
    try {
      const verifySupabase = getSafeSupabaseClient();
      if (!verifySupabase) {
        console.warn(
          "Skipping verification - database connection not available"
        );
      ...
      if (verifyError) {
        ...
        console.log("Verification of saved call score:", {
          ...
          match:
            verifyData.closing_score === numericScriptAdherence ? "YES" : "NO",
        });
        ...
        if (verifyData.closing_score !== numericScriptAdherence) {
          ...
          const { data: fixData, error: fixError } = await verifySupabase
            .from("call_scores")
...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      if (dbQueryError)
        console.error("Error in direct db query:", dbQueryError);
      ...
      const { data, error } = await supabase
        .from("call_scores")
        .select(
          "id, agent_id, manager_id, date, call_type, closing_score, notes"
        )
        .eq("agent_id", agentId)
        .order("date", { ascending: false });
    ...
    const transformedScores = callScores.map((score: any) => {
      ...
      if (score.closing_score !== undefined && score.closing_score !== null) {
        console.log(
          `Found closing_score: ${
            score.closing_score
          } (${typeof score.closing_score})`
        );
        numericClosingScore =
          typeof score.closing_score === "number"
            ? score.closing_score
      ...
      console.log(
        `Numeric closing score: ${numericClosingScore} (${typeof numericClosingScore})`
      );
    ...
    console.log(
      "Final transformedScores array before sending to client:",
    ...
    const finalScores = transformedScores.map((score: any) => {
      if (
        score.script_adherence === undefined ||
        score.script_adherence === null
      ) {
